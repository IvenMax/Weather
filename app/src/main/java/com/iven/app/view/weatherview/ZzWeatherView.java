package com.iven.app.view.weatherview;
...
public class ZzWeatherView extends HorizontalScrollView {
    ...
    private List<SevenDaysBean.HeWeather5Bean.DailyForecastBean> list;
            ...
            if (root.getChildCount() > 0) {
                ...
                if (lineType == LINE_TYPE_CURVE) {
                    ...
                    for (int i = 1; i < root.getChildCount(); i++) {
                        ...
                        pathDay.cubicTo(x01 + prevDx, y01 + prevDy, x1 - curDx, y1 - curDy, x1, y1);

                        pathNight.cubicTo(x02 + prevDx1, y02 + prevDy1, x2 - curDx1, y2 - curDy1, x2, y2);
    ...
    public List<SevenDaysBean.HeWeather5Bean.DailyForecastBean> getList() {
        return list;
    }
    ...
    public void setList(final List<SevenDaysBean.HeWeather5Bean.DailyForecastBean> list) {
        this.list = list;
        ...
        for (int i = 0; i < list.size(); i++) {
            SevenDaysBean.HeWeather5Bean.DailyForecastBean model = list.get(i);
            final WeatherItemView itemView = new WeatherItemView(getContext());
            itemView.setMaxTemp(max);
            itemView.setMinTemp(min);
            itemView.setDate(model.getDate().substring(model.getDate().length() - 2, model.getDate().length()) + "日");
//            itemView.setWeek(model.getDate().substring(model.getDate().length() - 2, model.getDate().length()) + "日");
            itemView.setDayTemp(Integer.parseInt(model.getTmp().getMax()));
            itemView.setDayWeather(model.getCond().getTxt_d());
            itemView.setDayImg(PicUtil.getDayWeatherPic(model.getCond().getTxt_d()));
            itemView.setNightWeather(model.getCond().getTxt_n());
            itemView.setNightTemp(Integer.parseInt(model.getTmp().getMin()));
            itemView.setNightImg(PicUtil.getDayWeatherPic(model.getCond().getTxt_n()));
            itemView.setWindOri(model.getWind().getDir());
            itemView.setWindLevel(model.getWind().getSc());
    ...
    private int getMinDayTemp(List<SevenDaysBean.HeWeather5Bean.DailyForecastBean> list) {
        if (list != null) {
            return Integer.parseInt(Collections.min(list, new DayTempComparator()).getTmp().getMax());
        }
        return 0;
    }

    private int getMinNightTemp(List<SevenDaysBean.HeWeather5Bean.DailyForecastBean> list) {
        if (list != null) {
            return Integer.parseInt(Collections.min(list, new NightTempComparator()).getTmp().getMin());
        }
        return 0;
    }


    private int getMaxNightTemp(List<SevenDaysBean.HeWeather5Bean.DailyForecastBean> list) {
        if (list != null) {
            return Integer.parseInt(Collections.max(list, new NightTempComparator()).getTmp().getMin());
        }
        return 0;
    }

    private int getMaxDayTemp(List<SevenDaysBean.HeWeather5Bean.DailyForecastBean> list) {
        if (list != null) {
            return Integer.parseInt(Collections.max(list, new DayTempComparator()).getTmp().getMax());
        }
        return 0;
    }
    ...
    private static class DayTempComparator implements Comparator<SevenDaysBean.HeWeather5Bean.DailyForecastBean> {

        @Override
        public int compare(SevenDaysBean.HeWeather5Bean.DailyForecastBean o1, SevenDaysBean.HeWeather5Bean.DailyForecastBean o2) {
            if (Integer.parseInt(o1.getTmp().getMax()) == Integer.parseInt(o2.getTmp().getMax())) {
                return 0;
            } else if (Integer.parseInt(o1.getTmp().getMax()) > Integer.parseInt(o2.getTmp().getMax())) {
                return 1;
            } else {
                return -1;
            }
    ...
    private static class NightTempComparator implements Comparator<SevenDaysBean.HeWeather5Bean.DailyForecastBean> {

        @Override
        public int compare(SevenDaysBean.HeWeather5Bean.DailyForecastBean o1, SevenDaysBean.HeWeather5Bean.DailyForecastBean o2) {
            if (Integer.parseInt(o1.getTmp().getMin()) == Integer.parseInt(o2.getTmp().getMin())) {
                return 0;
            } else if (Integer.parseInt(o1.getTmp().getMin()) > Integer.parseInt(o2.getTmp().getMin())) {
                return 1;
            } else {
                return -1;
            }
    ...
    public interface OnWeatherItemClickListener {
        void onItemClick(WeatherItemView itemView, int position, SevenDaysBean.HeWeather5Bean.DailyForecastBean dailyForecastBean);
    }
}
