...
public class LogisticsInformationView extends View {
     ...
     * 绘制界面参数
     */
    List<LogisticsData.DataBean> logisticsDataList;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < logisticsDataList.size(); i++) {
            ...
            if (i == 0) {
                ...
                mPaint.setColor(getResources().getColor(R.color.color_60cd89));
                //画文字
                mPaint.setTextSize(30);
                canvas.drawText(((LogisticsData.DataBean) data.get(i)).getTime() + "", left * 2 + radius * 2 + 10, heightList.get(i) + interval, mPaint);
                //文字换行
                TextPaint textPaint = new TextPaint();
                textPaint.setColor(getResources().getColor(R.color.color_60cd89));
                textPaint.setTextSize(35.0F);
                ...
                canvas.drawText(((LogisticsData.DataBean) data.get(i)).getTime() + "", left * 2 + radius * 2 + 10, interval + heightList.get(i) + heightData + top, paint);
                //文字换行
                TextPaint textPaint = new TextPaint();
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                for (int i = 0; i < stopYList.size(); i++) {
                    if (y < stopYList.get(i) && (stopYList.get(i) - y) <= phoneNumberHeight && x < stopYX.get(stopYList.get(i)) && (stopYX.get(stopYList.get(i)) - x) <= phoneNumberWidth) {
                        //成立,获取X轴的相关信息
