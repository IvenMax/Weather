...
public class URLImageGetter implements Html.ImageGetter {
    ...
    private Drawable getDrawableFromNet(final String source) {
        mNewsServiceRequest = RetrofitNewsUtil.getInstance().create(NewsServiceRequest.class);
        mNewsServiceRequest.getNewsBodyHtmlPhoto(source).unsubscribeOn(Schedulers.io()).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).map(new Func1<ResponseBody, Boolean>() {
            @Override
    ...
    private Drawable createPicPlaceholder() {
        Drawable drawable;
        int color = R.color.color_DEDEDE;
